...
async function startServer() {
  ...
  app.get("/sitemap.xml", async (_req, res) => {
    const catalog = await veloraDb.getCatalogContext();
    const base = process.env.PUBLIC_APP_URL || "https://velora.example";
    const staticPaths = ["/", "/blog", "/contact", "/policy/about"];
    const urls = [
      ...staticPaths.map((path) => `${base}${path}`),
      ...catalog.map((event) => `${base}/events/${event.slug}`),
    ];
  ...
  app.post("/api/scheduled/content", async (req, res) => {
    try {
      ...
      return res.json({ ok: true, skipped: result.skipped, article: result.article?.slug ?? null });
    } catch (error) {
